...
public:
  BasicStrategyBuild(std::string action) : _action(action) {}
    ...
    Action getAction(Hand* hand, int dealersUpcardRank) const override {
      if(dealersUpcardRank == 1) dealersUpcardRank += 10;
      if(!hand->isSoft()) {
	  if(hand->rank() >= 17) return Action::Stand;
	  if(hand->rank() >= 14) {
	  ...
	  if(hand->rank() == 12) {
        ...
        return _action == "Hit" ? Action::Hit :
	       _action == "Stand" ? Action::Stand :
	       _action == "Split" ? Action::Split :
	       _action == "DoubleDown" ? Action::DoubleDown : Action::Invalid;
    }
