...
public:
    ...
    Action getAction(Hand *hand, int dealersUpcardRank) const override
    {
        if (!hand->isSoft())
        {
            ...
            std::string playersHand = (hand->rank() <= 8) ? "8" : (hand->rank() >= 17) ? "17"
                                                                                       : std::to_string(hand->rank());
            ...
            else if (hand->rank() >= 12)
            {
                ...
                if (result == "B")
                    return _runningCount >= initialRunningCount ? Action::Stand : Action::Hit;
                if (result == "C")
                {
            ...
            else
            {
                ...
                if (result == "B")
                    return _runningCount >= initialRunningCount ? Action::DoubleDown : Action::Hit;
                if (result == "C")
                {
                    if (Rule::instance()->deck == 1)
                    {
